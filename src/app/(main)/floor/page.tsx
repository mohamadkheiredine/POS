...
export default function FloorPage() {
  ...
  const [editing, setEditing] = useState<{ open: boolean; table?: Table }>({
    open: false,
  });
  ...
  const bulkUpdate = (patch: Partial<Table>) =>
    setTables((arr) =>
      arr.map((t) => (selected.includes(t.id) ? { ...t, ...patch } : t))
    );
  ...
  const splitSelected = () => {
    ...
    setTables((arr) =>
      arr.map((t) =>
        selected.includes(t.id) ? { ...t, groupId: undefined } : t
      )
    );
  };
  ...
  const occupancy = useMemo(() => {
    ...
    const satSeats = vis
      .filter((t) => t.status === "seated" || t.status === "bill")
      .reduce((s, t) => s + t.seats, 0);
    return {
      totalSeats,
      satSeats,
      pct: totalSeats ? Math.round((satSeats / totalSeats) * 100) : 0,
    };
  }, [filtered]);
    ...
    <div className="min-h-[calc(100vh-4rem)] bg-white p-4">
      <div className="mx-auto grid w-full max-w-[1400px] gap-4 lg:grid-cols-[320px_1fr]">
        ...
        <aside className="overflow-hidden rounded-3xl bg-white/80 p-4 backdrop-blur-xl ring-1 ring-white/60 shadow-sm">
          ...
          <div className="space-y-2">
            ...
            <div className="grid gap-2">
              <div className="relative">
                ...
                >
                  {["All", "Main Hall", "Patio", "Bar", "VIP"].map((z) => (
              ...
              <div className="relative">
                ...
                <select
                  ...
                  className="w-full rounded-2xl border border-gray-200 bg-white pl-9 pr-8 py-2 text-sm"
                >
                  {["All", "vacant", "seated", "reserved", "bill", "dirty"].map(
                    (s) => (
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                onClick={addTable}
                className="inline-flex items-center justify-center gap-2 rounded-2xl border border-gray-200 bg-white px-3 py-2 text-sm hover:bg-gray-50"
              >
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button
                ...
                className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700"
              >
                Reserved
              </button>
              <button
                onClick={() => setTableStatus("bill")}
                className="rounded-2xl border border-purple-200 bg-purple-50 px-3 py-2 text-xs font-semibold text-purple-700"
              >
                Bill
              </button>
              <button
                onClick={() => setTableStatus("dirty")}
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-3 text-xs">
              ...
              <div className="mt-1 text-gray-800">
                {occupancy.satSeats}/{occupancy.totalSeats} seats ·{" "}
                {occupancy.pct}%
              </div>
            </div>
          </div>
        ...
        <main className="overflow-hidden rounded-3xl bg-white/60 p-4 backdrop-blur-xl ring-1 ring-white/60 shadow-sm">
          ...
          {/* Floor plan area */}
          <div className="relative h-[72vh] w-full overflow-hidden rounded-2xl border border-gray-200 bg-[linear-gradient(90deg,rgba(0,0,0,0.03)_1px,transparent_1px),linear-gradient(rgba(0,0,0,0.03)_1px,transparent_1px)] bg-[size:24px_24px]">
            <div
              ...
              {filtered.map((t) => (
                ...
                >
                  ...
                  <div className="flex h-full flex-col items-center justify-center p-1">
                    ...
                    <div
                      className="mt-0.5 inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[10px] font-semibold ring-1 ring-inset
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-gray-700">
            <span className="rounded-full bg-emerald-100/80 px-2 py-0.5">
              Vacant
            </span>
            <span className="rounded-full bg-blue-100/80 px-2 py-0.5">
              Seated
            </span>
            <span className="rounded-full bg-amber-100/80 px-2 py-0.5">
              Reserved
            </span>
            <span className="rounded-full bg-purple-100/80 px-2 py-0.5">
              Bill
            </span>
            <span className="rounded-full bg-rose-100/80 px-2 py-0.5">
              Dirty
            </span>
            <span className="ml-auto text-xs text-gray-500">
              Tip: Shift+Click to multi-select. Double-click a table to edit.
            </span>
          </div>
        </main>
      </div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-4">
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div>
                ...
                <select
                  value={form.zone as Zone}
                  onChange={(e) => set({ zone: e.target.value as Zone })}
                  className="mt-1 w-full rounded-lg border border-gray-200 bg-white px-3 py-2"
                >
                  {["Main Hall", "Patio", "Bar", "VIP"].map((z) => (
                    <option key={z}>{z}</option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <select
                  ...
                  className="mt-1 w-full rounded-lg border border-gray-200 bg-white px-3 py-2"
                >
                  {["vacant", "seated", "reserved", "bill", "dirty"].map(
                    (s) => (
                      <option key={s}>{s}</option>
                    )
                  )}
