...
export default function UsersPage() {
  ...
  const fetchRoles = async () => {
    const { data } = await api.get(`${API}/request/api/lists/roles`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setRoles(data.items));
  };
  const fetchUserTypes = async () => {
    const { data } = await api.get(`${API}/request/api/lists/usertypes`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setUserTypes(data.items));
  };
  const fetchCompanies = async () => {
    const { data } = await api.get(`${API}/request/api/lists/companies`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setCompanies(data.items));
  };
  const fetchLangs = async () => {
    const { data } = await api.get(`${API}/request/api/lists/langs`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setLangs(data.items));
  };
  const fetchJobTitles = async () => {
    const { data } = await api.get(`${API}/request/api/lists/jobtitles`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setJobTitles(data.items));
  };
  const fetchJobRoles = async () => {
    const { data } = await api.get(`${API}/request/api/lists/jobroles`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setJobRoles(data.items));
  };
  const fetchDepartments = async () => {
    const { data } = await api.get(`${API}/request/api/lists/departments`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setDepartments(data.items));
  };
  const fetchWarehouses = async () => {
    const { data } = await api.get(`${API}/request/api/lists/warehouses`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setWarehouses(data.items));
  };
  const fetchEmploymentTypes = async () => {
    const { data } = await api.get(`${API}/request/api/lists/employmenttypes`, {
      params: authParams,
    });
    if (data.is_error === 0) dispatch(setEmploymentTypes(data.items));
  };
  const fetchPaymentTypes = async () => {
    const { data } = await api.get(`${API}/request/api/lists/paymenttypes`, {
      params: authParams,
    });
  ...
  const fetchEditUserData = async (editUserId: number) => {
    try {
      const { data } = await api.get(`${API}/request/api/getusereditdata`, {
  ...
  const openEdit = async (u: UserRecord) => {
    ...
    if (editData) {
      ...
      setForm((f) => ({
        ...
        u_date_birth: ui.u_date_birth
          ? String(ui.u_date_birth).split("T")[0]
          : "",
        u_fax: ui.u_fax || "",
        ...
        u_employment_date: ui.u_employment_date
          ? String(ui.u_employment_date).split("T")[0]
          : "",
  ...
  const handleSave = async () => {
    ...
    if (!form.u_employment_date)
      errs.u_employment_date = "Employment date is required";
    ...
    try {
      ...
      const { data } = await api.post(`${API}/request/api/saveuser`, fd);
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="col-span-2">
                      <Field label="Full Name *">
                        <input
                          value={form.u_fullname}
                          onChange={setField("u_fullname")}
                          className={inputCls(fieldErrors.u_fullname)}
                          placeholder="John Doe"
                        />
                        {fieldErrors.u_fullname && (
                          <ErrMsg msg={fieldErrors.u_fullname} />
                        )}
                      </Field>
                    </div>
                    ...
                    <div>
                      <Field label="Username *">
                        <input
                          ...
                          onChange={setField("u_username")}
                          className={inputCls(fieldErrors.u_username)}
                          placeholder="johndoe"
                        />
                        {fieldErrors.u_username && (
                          <ErrMsg msg={fieldErrors.u_username} />
                        )}
                      </Field>
                    </div>
                    ...
                    <div>
                      <Field label="Attendance PIN (5-digit)">
                        <input
                          value={form.u_attendance_code}
                          onChange={setField("u_attendance_code")}
                          className={INPUT}
                          maxLength={5}
                          placeholder="12345"
                        />
                    ...
                    <div>
                      <Field label="User Type *">
                        <select
                          value={form.u_user_type}
                          onChange={setField("u_user_type")}
                          className={inputCls(fieldErrors.u_user_type)}
                        >
                          <option value="">— Select —</option>
                          {userTypesState.items.map((t) => (
                            <option key={t.ut_id} value={t.ut_id}>
                              {t.ut_user_type}
                            </option>
                          ))}
                        </select>
                        {fieldErrors.u_user_type && (
                          <ErrMsg msg={fieldErrors.u_user_type} />
                        )}
                      </Field>
                    </div>
                    ...
                    <div>
                      <Field label="Role *">
                        <select
                          value={form.fk_role_id}
                          onChange={setField("fk_role_id")}
                          className={inputCls(fieldErrors.fk_role_id)}
                        >
                          <option value="">— Select —</option>
                        ...
                        {fieldErrors.fk_role_id && (
                          <ErrMsg msg={fieldErrors.fk_role_id} />
                        )}
                      </Field>
                    </div>
                    ...
                    <div>
                      <Field label="Language">
                        <select
                          value={form.u_lang_id}
                          onChange={setField("u_lang_id")}
                          className={INPUT}
                        >
                          <option value="">— Select —</option>
                          {langsState.items.map((l) => (
                            <option key={l.lm_id} value={l.lm_id}>
                              {l.lm_lang_name}
                            </option>
                          ))}
                        </select>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    <Field label="Date of Birth *">
                      <input
                        type="date"
                        value={form.u_date_birth}
                        onChange={setField("u_date_birth")}
                        className={inputCls(fieldErrors.u_date_birth)}
                      />
                      {fieldErrors.u_date_birth && (
                        <ErrMsg msg={fieldErrors.u_date_birth} />
                      )}
                    </Field>
                  </div>
                  ...
                  <div>
                    <Field label="Email *">
                      <input
                        type="email"
                        value={form.u_email}
                        onChange={setField("u_email")}
                        className={inputCls(fieldErrors.u_email)}
                        placeholder="john@example.com"
                      />
                      {fieldErrors.u_email && (
                        <ErrMsg msg={fieldErrors.u_email} />
                      )}
                    </Field>
                  </div>
                  ...
                  <div className="col-span-2">
                    <Field label="Website">
                      <input
                        value={form.u_website}
                        onChange={setField("u_website")}
                        className={INPUT}
                  ...
                  <div>
                    <Field label="Marital Status">
                      <select
                        value={form.u_marital_status}
                        onChange={setField("u_marital_status")}
                        className={INPUT}
                      >
                        <option value="">— Select —</option>
                  ...
                  <div>
                    <Field label="No. of Dependents">
                      <input
                        type="number"
                        min="0"
                        value={form.u_number_of_dependencies}
                        onChange={setField("u_number_of_dependencies")}
                        className={INPUT}
                        placeholder="0"
                      />
                    </Field>
                  ...
                  <div className="col-span-2">
                    <Field label="Residential Area">
                      <input
                        value={form.u_residential_area}
                        onChange={setField("u_residential_area")}
                        className={INPUT}
              ...
              {activeTab === "employment" && (
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Field label="Company">
                      <select
                        value={form.fk_company_id}
                        onChange={setField("fk_company_id")}
                        className={INPUT}
                      >
                        <option value="">— Select —</option>
                        {companiesState.items.map((c) => (
                          <option key={c.cd_id} value={c.cd_id}>
                            {c.cd_company_name}
                          </option>
                        ))}
                      </select>
                  ...
                  <div>
                    <Field label="Department">
                      <select
                        value={form.u_department_id}
                        onChange={setField("u_department_id")}
                        className={INPUT}
                      >
                        <option value="">— Select —</option>
                        {departmentsState.items.map((d) => (
                          <option key={d.sd_id} value={d.sd_id}>
                            {d.sd_department_title}
                          </option>
                        ))}
                      </select>
                    </Field>
                  ...
                  <div>
                    <Field label="Warehouse">
                      <select
                        value={form.fk_warehouse_id}
                        onChange={setField("fk_warehouse_id")}
                        className={INPUT}
                      >
                        <option value="">— Select —</option>
                  ...
                  <div>
                    <Field label="Job Role">
                      <select
                        value={form.u_job_role_id}
                        onChange={setField("u_job_role_id")}
                        className={INPUT}
                      >
                        <option value="">— Select —</option>
                        {jobRolesState.items.map((r) => (
                          <option key={r.jr_id} value={r.jr_id}>
                            {r.jr_job_role}
                          </option>
                        ))}
                      </select>
                  ...
                  <div>
                    <Field label="Job Title">
                      <select
                        value={form.u_job_title_id}
                        onChange={setField("u_job_title_id")}
                        className={INPUT}
                      >
                        <option value="">— Select —</option>
                        {jobTitlesState.items.map((t) => (
                          <option key={t.jt_id} value={t.jt_id}>
                            {t.jt_job_title}
                          </option>
                        ))}
                      </select>
                  ...
                  <div>
                    <Field label="Employee Type">
                      <select
                        value={form.u_employee_type}
                        onChange={setField("u_employee_type")}
                        className={INPUT}
                      >
                        <option value="">— Select —</option>
                        {employmentTypesState.items.map((e) => (
                          <option key={e.et_id} value={e.et_id}>
                            {e.et_type}
                          </option>
                        ))}
                      </select>
                  ...
                  <div>
                    <Field label="Employment Date *">
                      <input
                        type="date"
                        value={form.u_employment_date}
                        onChange={setField("u_employment_date")}
                        className={inputCls(fieldErrors.u_employment_date)}
                      />
                      {fieldErrors.u_employment_date && (
                        <ErrMsg msg={fieldErrors.u_employment_date} />
                      )}
                    </Field>
                  </div>
                  ...
                  <div>
                    <Field label="Daily Working Hours">
                      <input
                        ...
                        step="0.5"
                        min="0"
                        value={form.u_daily_working_hours}
                        onChange={setField("u_daily_working_hours")}
                        className={INPUT}
                        placeholder="9.5"
                      />
                    </Field>
                  ...
                  <div>
                    <Field label="Salary">
                      <input
                        type="number"
                        min="0"
                        value={form.u_user_sallary}
                        onChange={setField("u_user_sallary")}
                        className={INPUT}
                        placeholder="0.00"
                      />
                    </Field>
                  ...
                  <div>
                    <Field label="Sales Commission (%)">
                      <input
                        ...
                        min="0"
                        max="100"
                        value={form.u_sales_commission}
                        onChange={setField("u_sales_commission")}
                        className={INPUT}
                        placeholder="0"
                      />
                    </Field>
                  ...
                  <div>
                    <Field label="Hourly Rate">
                      <input
                        type="number"
                        min="0"
                        value={form.u_hourly_rate}
                        onChange={setField("u_hourly_rate")}
                        className={INPUT}
                        placeholder="0.00"
                      />
                    </Field>
                  ...
                  <div>
                    <Field label="Number of Holidays">
                      <input
                        type="number"
                        min="0"
                        value={form.u_number_holidays}
                        onChange={setField("u_number_holidays")}
                        className={INPUT}
                        placeholder="0"
                      />
                    </Field>
                  ...
                  <div>
                    <Field label="CNSS Number">
                      <input
                        value={form.u_cnss_number}
                        onChange={setField("u_cnss_number")}
                        className={INPUT}
                  ...
                  <div>
                    <Field label="Payment Method">
                      <select
                        value={form.pm_payment_method}
                        onChange={setField("pm_payment_method")}
                        className={INPUT}
                      >
                        <option value="">— Select —</option>
                  ...
                  <div className="col-span-2 flex items-center gap-2 pt-1">
                    <input
                      ...
                      onChange={(e) =>
                        setForm((f) => ({
                          ...f,
                          u_has_insurance: e.target.checked,
                        }))
                      }
                      className="h-4 w-4 rounded border-gray-300 text-indigo-600"
                    />
                    <label
                      htmlFor="u_has_insurance"
                      className="text-sm text-gray-700 select-none"
                    >
                      Has Insurance
                    </label>
